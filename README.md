...
ICHI 5 部門の自動化構造を Obsidian Vault として出力し、グラフビューやダッシュボードで確認できます。

### セットアップ

```bash
# 1. .env に Vault パスを設定
# OBSIDIAN_VAULT_DIR=C:/Users/yourname/Documents/ICHI-vault

# 2. Vault を生成
npm run obsidian:sync

# 3. Obsidian で開く
# File > Open folder as vault > OBSIDIAN_VAULT_DIR を選択
# コミュニティプラグイン > Dataview をインストール & 有効化
```
...
| フォルダ | 内容 |
|---|---|
| `00-index.md` | Vault トップページ (全体概要) |
| `01-departments/` | 5 部門ごとの自動化率・スクリプト・ワークフロー |
| `02-scripts/` | 全 npm scripts の詳細 |
| `03-workflows/` | 全 GitHub Actions ワークフロー |
| `04-data-flows/` | Mermaid 図でデータフローを表現 |
| `05-dashboard/` | Dataview クエリで動的ダッシュボード |
| `06-reference/` | 環境変数・Secrets・税率一覧 |

### 運用

- 構造変化があったら `npm run obsidian:sync` を再実行（月 1 回程度で十分）
- `auto_generated: true` のノートは再実行で自動上書き
- 手書きメモを追加したいノートは `auto_generated: false` を frontmatter に設定

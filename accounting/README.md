...
## Obsidian 連携 (`npm run obsidian:sync`)

`audit.js` が生成した `AuditReport` を Obsidian のノート群に変換します。
経理・営業・技術・開発企画・広報の全 5 部門を部門ノート・スクリプトノート・
ワークフローノート・データフロー図・ダッシュボードとして出力します。

```bash
npm run obsidian:sync            # 通常実行
npm run obsidian:sync -- --dry-run   # 生成予定を確認
npm run obsidian:sync -- --force     # 手書きノートも上書き
```

詳細はリポジトリ直下の README.md「Obsidian で構造を可視化」セクションを参照。

---

## 自動化棚卸しレポート (`npm run audit`)

経理部門を含む全部門の自動化範囲を、リポジトリの実コードから事実ベースで生成するレポートです。
...
| ファイル | 内容 |
|---|---|
| `outputs/audit/automation-coverage-<YYYY-MM-DD>.md` | Markdown 棚卸しレポート (人が読む用) |
| `outputs/audit/automation-coverage-<YYYY-MM-DD>.json` | JSON (機械可読・差分追跡用) |

データリポジトリ分離が有効な場合は `ichi-data/accounting/outputs/audit/` に生成されます。

### レポートの読み方

- **エグゼクティブサマリ**: 全体の自動化率と機微情報分離状況
- **部門別詳細**: 完全自動 / AI 補助 / 人手タスクの分類と既知ギャップ
- **自動化率**: `(完全自動 × 1.0 + AI 補助 × 0.7) ÷ スクリプト数` で算出
- AI による推測は最小限。不明な箇所は `"(README に未記載)"` と明示

---

## データの保管場所 (ichi-data)

機微情報を含むファイルは **ichi-data プライベートリポジトリ** で管理します。

| 場所 | 内容 | 管理 |
|---|---|---|
| `ichi-data/accounting/inputs/` | 請求書 MD, payables, 銀行 CSV, 領収書 | gitignore: ichi-blog 非追跡 |
| `ichi-data/accounting/state/` | 採番カウンタ, 消込履歴, 処理済記録 | Actions が自動コミット |
| `ichi-data/accounting/outputs/` | PDF, 月次レポート, 税理士パッケージ | Actions が自動コミット |

### ローカル vs Actions 対比

| 項目 | ローカル運用 | Actions 運用 |
|---|---|---|
| ichi-data 取得 | `ln -s ../ichi-data data` | checkout-data-repo composite で自動取得 |
| 入力ファイル | ichi-data に直接配置 | ichi-data に push 後、Actions が参照 |
| state 更新 | ローカルで更新、git push は手動 | commit-data-changes composite で自動コミット |
| 出力ファイル | `data/accounting/outputs/` に生成 | ichi-data に自動コミット |

INPUT_BASE_DIR が未設定 (ichi-data 未連携) の場合は ichi-blog 配下をそのまま使用します (後方互換)。
